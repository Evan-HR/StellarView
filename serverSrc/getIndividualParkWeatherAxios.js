...
async function getIndividualParkWeatherAxios(park, userTime, weatherKey1) {
	...
	if (!forecast) {
		...
		park.weather = {
			...
			stationDist: geolib.getDistance({
				lat: park.lat,
				lng: park.lng,
			}, {
				lat: response.coord.lat,
		...
		for (var i = 0; i < response.cnt; i++) {
			console.log(new Date(response.list[i].dt_txt).getTime(), nightTime.getTime(), new Date(response.list[i].dt_txt).getTime() >
				nightTime.getTime());
			if (new Date(response.list[i].dt_txt).getTime() >
				nightTime.getTime()) {
				//If the date before nightfall is closer to nightfall than the one after, pick the closer one
				let succInst = i;
				if (i > 0 &&
					Math.abs(new Date(response.list[i - 1].dt_txt) -
						nightTime.getTime()) <
					Math.abs(new Date(response.list[i].dt_txt).getTime() -
						nightTime.getTime())) {
					succInst = i - 1;
				}
				console.log("Success! Looking at ", succInst, ":", response.list[succInst]);
