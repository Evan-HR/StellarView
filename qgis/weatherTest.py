...
simulatedParks = np.random.uniform(low=[minLng, minLat], high=[
                                   maxLng, maxLat], size=(20, 2))
...
edge2 = np.array([[edge_x[-1, i], edge_y[-1, i]]
...
edge4 = np.array([[edge_x[i, -1], edge_y[i, -1]]
                  for i in range(0, len(edge_x))])
...
plt.plot(simulatedParks[:, 0], simulatedParks[:, 1],
...
for i in range(0, len(simulatedParks)):
    plt.text(simulatedParks[i][0], simulatedParks[i][1],
             f'{round(parkDataNN[i],1)}')
plt.ylabel('lat')
plt.xlabel('lng')
...
for i in range(0, len(simulatedParks)):
    plt.text(simulatedParks[i][0], simulatedParks[i]
             [1], f'{round(parkDataLI[i],1)}')
plt.ylabel('lat')
plt.xlabel('lng')
